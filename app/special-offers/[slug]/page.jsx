...
export default async function OfferDetail({ params }) {
  const item = await readItemBySlug("special_offers", params.slug);
  if (!item) return <div className="panel">Не найдено.</div>;

  return (
    <div>
      <div className="kicker"><Link href="/special-offers">Акции</Link> / {item.title}</div>
      ...
      <div className="panel">
        ...
        <div style={{marginTop:14}}><Link className="button" href="/special-offers">К списку</Link></div>
      </div>
    </div>
  );
}
